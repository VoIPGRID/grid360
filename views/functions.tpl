{function showActions}
    ...
    {if {$type} == "user"}
        <form action="{${$level}_uri}/{$type}/{${$type}.id}">
            {if $user.status == 1}
                <button data-toggle="tooltip" title="Pause feedback" data-placement="bottom" type="submit"
                        class="btn btn-link">
                    <i class="icon-pause"></i>
                </button>
            {else}
                <button data-toggle="tooltip" title="Resume feedback" data-placement="bottom" type="submit"
                        class="btn btn-link">
                    <i class="icon-play"></i>
                </button>
            {/if}
